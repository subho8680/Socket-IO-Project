/**
 * ContestCreator.jsx
 * Contest creation with email invites, scheduled start, visibility settings.
 *
 * npm install antd @ant-design/icons framer-motion dayjs
 */

import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import {
    ConfigProvider, message, DatePicker,
    Input, Select, Button, Switch,
} from "antd";
import {
    TrophyOutlined, ThunderboltOutlined, CodeOutlined,
    LockOutlined, CheckCircleFilled, CopyOutlined,
    TeamOutlined, ClockCircleOutlined, FireOutlined,
    MailOutlined, GlobalOutlined, EyeInvisibleOutlined,
    PlusOutlined, CloseOutlined, CalendarOutlined,
    RocketOutlined,
...
const TAGS = [
    ...
    "strings", "binary search", "sortings", "number theory",
    "two pointers", "trees", "geometry", "combinatorics",
];

const RATINGS = [800, 900, 1000, 1100, 1200, 1300, 1400, 1500,
    1600, 1700, 1800, 1900, 2000, 2200, 2400, 2600, 3000];

const LOADING_STEPS = [
    { icon: <CodeOutlined />, label: "Scanning problem database", sub: "Querying Codeforces API…" },
    { icon: <ThunderboltOutlined />, label: "Running selection algorithm", sub: "Picking the perfect problems…" },
    { icon: <FireOutlined />, label: "Fetching test cases", sub: "Collecting sample I/O…" },
    { icon: <LockOutlined />, label: "Sealing the contest", sub: "Locking problems until start…" },
];
...
export default function ContestCreator({ onContestCreated, currentUser }) {
    ...
    const barWidth = ((form.maxRating - form.minRating) / SPAN * 100).toFixed(1);

    const addEmail = () => {
        const e = emailInput.trim().toLowerCase();
        if (!e) return;
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)) { messageApi.error("Invalid email address"); return; }
        if (form.inviteEmails.includes(e)) { messageApi.warning("Already added"); return; }
        set("inviteEmails", [...form.inviteEmails, e]);
        setEmailInput("");
    };

    const removeEmail = (e) => set("inviteEmails", form.inviteEmails.filter(x => x !== e));
    ...
    const handleCreate = async () => {
        ...
        try {
            const pRes = await fetch(`${API}/contest/preview-problems`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ minR: form.minRating, maxR: form.maxRating, count: form.count, tags: form.tags }),
            });
            const pData = await pRes.json();
            if (pData.status !== "success") throw new Error(pData.message);
            setCompletedSteps([0]); await pause(350);

            setCurrentStep(1); await pause(900);
            setCompletedSteps(p => [...p, 1]); await pause(350);

            setCurrentStep(2);
            const sRes = await fetch(`${API}/contest/scrape`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    name: form.name, description: form.description,
                    durationMinutes: form.duration, scoring: form.scoring,
                    problems: pData.problems,
                    cfHandles: form.cfHandles.split(",").map(h => h.trim()).filter(Boolean),
                    inviteEmails: form.inviteEmails,
                    scheduledAt: form.scheduledAt ? form.scheduledAt.toISOString() : null,
                    visibility: form.visibility, allowLateJoin: form.allowLateJoin,
                    createdBy: currentUser?.email ?? "anonymous",
                }),
            });
            const sData = await sRes.json();
            if (!sData.success) throw new Error(sData.message);
            setCompletedSteps(p => [...p, 2]); await pause(350);

            setCurrentStep(3); await pause(900);
            setCompletedSteps(p => [...p, 3]); await pause(500);
    ...
    const resetForm = () => {
        setPhase("form"); setContestResult(null);
        setForm({
            name: "", minRating: 1000, maxRating: 1600, count: 5, duration: 120,
            scoring: "icpc", tags: [], cfHandles: "", inviteEmails: [],
            scheduledAt: null, visibility: "private", description: "", allowLateJoin: true,
        });
    ...
    return (
        <ConfigProvider theme={{
            token: {
                ...
                colorBgElevated: "#FFFFFF",      /* popups, dropdowns, calendar panel */
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {phase === "form" && (
                            <motion.div key="form"
                                ...
                                exit={{ opacity: 0, y: -20 }} transition={{ duration: .4, ease: [.4, 0, .2, 1] }}
                            >
                                {/* Header */}
                                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .1 }} style={{ marginBottom: 28 }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                                        <div style={{ width: 40, height: 40, borderRadius: 12, background: "linear-gradient(135deg,#4F46E5,#7C3AED)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 12px rgba(79,70,229,.3)" }}>
                                            <TrophyOutlined style={{ color: "#fff", fontSize: 18 }} />
                                        </div>
                                        <div>
                                            <div style={{ fontSize: 11, fontWeight: 700, color: "#4F46E5", textTransform: "uppercase", letterSpacing: ".08em", fontFamily: "'Outfit',sans-serif" }}>CodeClash</div>
                                            <div style={{ fontSize: 12, color: "#9CA3AF", fontFamily: "'Outfit',sans-serif" }}>Contest Platform</div>
                                        </div>
                                    </div>
                                    <h1 style={{ fontSize: 28, fontWeight: 800, color: "#0F172A", margin: "0 0 8px", letterSpacing: "-0.5px", lineHeight: 1.2, fontFamily: "'Outfit',sans-serif" }}>Create your contest</h1>
                                    <p style={{ color: "#6B7280", fontSize: 14, margin: 0, lineHeight: 1.6, fontFamily: "'Outfit',sans-serif" }}>Configure rules and filters — problems are auto-selected and sealed until you start.</p>
                                </motion.div>

                                {/* ── Card 1: Basic Info ── */}
                                <motion.div className="cc-card" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .15 }}>
                                    <div className="cc-sec">
                                        <div className="cc-sec-title"><CodeOutlined style={{ fontSize: 10 }} />Basic Info</div>
                                        ...
                                        <div>
                                            <div className="cc-label">
                                                Description
                                                <span className="cc-hint">(optional — shown to participants)</span>
                                            </div>
                                            <TextArea
                                                value={form.description}
                                                onChange={e => set("description", e.target.value)}
                                                placeholder="What's this contest about? Any special rules?"
                                ...
                                {/* ── Card 2: Schedule & Visibility ── */}
                                <motion.div className="cc-card" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .2 }}>
                                    <div className="cc-sec">
                                        <div className="cc-sec-title"><CalendarOutlined style={{ fontSize: 10 }} />Schedule & Visibility</div>

                                        <div className="cc-g2" style={{ marginBottom: 16 }}>
                                            <div>
                                                <div className="cc-label"><ClockCircleOutlined style={{ fontSize: 10 }} />Scheduled start</div>
                                                <DatePicker
                                            ...
                                            <div>
                                                <div className="cc-label"><ClockCircleOutlined style={{ fontSize: 10 }} />Duration</div>
                                                <Select
                                                    value={form.duration}
                                                    ...
                                                    size="large"
                                                >
                                                    {[[60, "1 hour"], [90, "1.5 hours"], [120, "2 hours"], [150, "2.5 hours"], [180, "3 hours"], [240, "4 hours"]].map(([v, l]) => (
                                        ...
                                        <div>
                                            <div className="cc-label"><GlobalOutlined style={{ fontSize: 10 }} />Visibility</div>
                                            <div style={{ display: "flex", gap: 10 }}>
                                                <button className={`cc-vis-btn ${form.visibility === "private" ? "on" : ""}`} onClick={() => set("visibility", "private")}>
                                                    <EyeInvisibleOutlined style={{ fontSize: 14 }} /> Private
                                                </button>
                                                <button className={`cc-vis-btn ${form.visibility === "public" ? "on" : ""}`} onClick={() => set("visibility", "public")}>
                                                    <GlobalOutlined style={{ fontSize: 14 }} /> Public
                                                </button>
                                            </div>
                                            <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 8, fontFamily: "'Outfit',sans-serif" }}>
                                                {form.visibility === "private"
                                                    ? "Only invited participants can see and join this contest"
                                                    : "Anyone with the link can discover and join this contest"}
                                            </div>
                                        </div>

                                        <div className="cc-toggle-row">
                                            <div>
                                                <div className="cc-toggle-label">Allow late join</div>
                                                <div className="cc-toggle-sub">Participants can join after contest starts</div>
                                            </div>
                                            <Switch
                                ...
                                {/* ── Card 3: Problem Config ── */}
                                <motion.div className="cc-card" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .25 }}>
                                    <div className="cc-sec">
                                        <div className="cc-sec-title"><ThunderboltOutlined style={{ fontSize: 10 }} />Problem Config</div>
                                        ...
                                        <div style={{ marginBottom: 16 }}>
                                            ...
                                            <div className="cc-g2">
                                                <div>
                                                    ...
                                                    >
                                                        {RATINGS.filter(r => r < form.maxRating).map(r => (
                                                ...
                                                <div>
                                                    ...
                                                    <Select
                                                        ...
                                                        popupMatchSelectWidth={false}
                                                    >
                                                        {RATINGS.filter(r => r > form.minRating).map(r => (
                                                            <Option key={r} value={r}>{r}</Option>
                                                        ))}
                                                    </Select>
                                                </div>
                                            ...
                                            <div className="cc-rbar">
                                                <motion.div className="cc-rfill"
                                                    animate={{ left: `${barLeft}%`, width: `${barWidth}%` }}
                                                    style={{ background: `linear-gradient(90deg,${getRatingColor(form.minRating).bar},${getRatingColor(form.maxRating).bar})` }}
                                                    transition={{ duration: .3 }}
                                                />
                                            </div>
                                            <div className="cc-rlabels">
                                                <span style={{ color: getRatingColor(form.minRating).bar }}>{form.minRating}</span>
                                                <span style={{ color: "#C4C9D4", fontWeight: 400, fontSize: 11 }}>CF difficulty scale</span>
                                                <span style={{ color: getRatingColor(form.maxRating).bar }}>{form.maxRating}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="cc-g2">
                                            <div>
                                                ...
                                                <Select value={form.count} onChange={v => set("count", v)} style={{ width: "100%" }} size="large">
                                                    {[3, 4, 5, 6, 7, 8].map(n => <Option key={n} value={n}>{n} problems</Option>)}
                                                </Select>
                                            </div>
                                            <div>
                                                <div className="cc-label">Scoring</div>
                                                <Select value={form.scoring} onChange={v => set("scoring", v)} style={{ width: "100%" }} size="large">
                                    ...
                                    <div className="cc-sec">
                                        <div className="cc-label">
                                            <FireOutlined style={{ fontSize: 10 }} />Topics
                                            <span className="cc-hint">(optional)</span>
                                        </div>
                                        <div style={{ display: "flex", flexWrap: "wrap", margin: "-3px" }}>
                                            {TAGS.map(t => (
                                                <motion.span key={t} className={`cc-tag ${form.tags.includes(t) ? "on" : ""}`}
                                                    onClick={() => toggleTag(t)} whileTap={{ scale: .95 }}>
                                                    {t}
                                                </motion.span>
                                            ))}
                                        </div>
                                ...
                                <motion.div className="cc-card" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .3 }}>
                                    <div className="cc-sec">
                                        <div className="cc-sec-title"><TeamOutlined style={{ fontSize: 10 }} />Participants</div>

                                        <div style={{ marginBottom: 16 }}>
                                            <div className="cc-label">
                                                <MailOutlined style={{ fontSize: 10 }} />Invite by email
                                                <span className="cc-hint">(send contest link automatically)</span>
                                            </div>
                                            <div className="cc-email-row">
                                                <Input
                                                    ...
                                                    size="large"
                                                    prefix={<MailOutlined style={{ color: "#C4C9D4", fontSize: 13 }} />}
                                                    style={{ flex: 1 }}
                                                />
                                                <Button
                                                    icon={<PlusOutlined />}
                                            ...
                                            {form.inviteEmails.length > 0 && (
                                                <motion.div
                                                    ...
                                                    style={{ marginTop: 10, display: "flex", flexWrap: "wrap", gap: 2 }}
                                                >
                                                    {form.inviteEmails.map(e => (
                                                        <span key={e} className="cc-email-pill">
                                                            <MailOutlined style={{ fontSize: 10 }} />
                                                            {e}
                                                            <span className="cc-email-x" onClick={() => removeEmail(e)}><CloseOutlined /></span>
                                                        </span>
                                                    ))}
                                                </motion.div>
                                            )}
                                        ...
                                        <div>
                                            <div className="cc-label">
                                                <TeamOutlined style={{ fontSize: 10 }} />CF handles
                                                <span className="cc-hint">(for submission polling, comma-separated)</span>
                                            </div>
                                            <Input
                                                value={form.cfHandles}
                                                onChange={e => set("cfHandles", e.target.value)}
                                                placeholder="tourist, jiangly, Petr"
                                                size="large"
                                ...
                                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .35 }}>
                                    ...
                                    <p style={{ textAlign: "center", fontSize: 12, color: "#B0B8C8", marginTop: 10, fontFamily: "'Outfit',sans-serif" }}>
                                        Problems are sealed — participants won't see them until contest start
                                    </p>
                                </motion.div>
                            </motion.div>
                        )}
                        ...
                        {phase === "loading" && (
                            ...
                            >
                                <motion.div style={{ display: "flex", justifyContent: "center", marginBottom: 32 }}
                                ...
                                >
                                    <div style={{ position: "relative", width: 96, height: 96 }}>
                                        {[0, 1, 2].map(i => (
                                            <motion.div key={i}
                                                style={{ position: "absolute", inset: i * 10, borderRadius: "50%", border: "1.5px solid rgba(79,70,229,.2)" }}
                                                animate={{ scale: [1, 1.1, 1], opacity: [.3, .1, .3] }}
                                                transition={{ duration: 2.5, repeat: Infinity, delay: i * .4 }}
                                            />
                                        ))}
                                        <div style={{ position: "absolute", inset: 22, borderRadius: "50%", border: "3px solid #EEF2FF", borderTopColor: "#4F46E5", animation: "cc-spin .85s linear infinite" }} />
                                        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26 }}>🔒</div>
                                    </div>
                                </motion.div>

                                <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .2 }}
                                    style={{ fontSize: 24, fontWeight: 800, color: "#0F172A", margin: "0 0 8px", letterSpacing: "-0.5px", fontFamily: "'Outfit',sans-serif" }}>
                                    Building your contest
                                </motion.h2>
                                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: .3 }}
                                    style={{ color: "#6B7280", fontSize: 14, margin: "0 0 32px", lineHeight: 1.6, fontFamily: "'Outfit',sans-serif" }}>
                                    Problems are being selected in the background.<br />They'll only be revealed when you start.
                                </motion.p>

                                <motion.div className="cc-card" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .3 }} style={{ textAlign: "left" }}>
                                    {LOADING_STEPS.map((step, i) => {
                                        const isDone = completedSteps.includes(i);
                                        const isActive = currentStep === i && !isDone;
                                        const isPending = i > currentStep;
                                        return (
                                            <motion.div key={i} className="cc-step-row"
                                                initial={{ opacity: 0, x: -10 }} animate={{ opacity: isPending ? .35 : 1, x: 0 }}
                                                transition={{ delay: i * .08 }}
                                                style={{ background: isActive ? "#FAFAFA" : "transparent" }}
                                            >
                                                <div className="cc-step-icon" style={{
                                                    background: isDone || isActive ? "#EEF2FF" : "#F9FAFB",
                                                    border: `2px solid ${isDone || isActive ? "#4F46E5" : "#E5E7EB"}`,
                                                    color: isDone || isActive ? "#4F46E5" : "#9CA3AF",
                                                }}>
                                                    {isDone ? (
                                                        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 400 }}>
                                                            <CheckCircleFilled style={{ color: "#4F46E5", fontSize: 16 }} />
                                                        </motion.div>
                                                    ) : isActive
                                                ...
                                                <div style={{ flex: 1 }}>
                                                    <div style={{ fontSize: 14, fontWeight: isDone || isActive ? 600 : 400, color: isPending ? "#9CA3AF" : "#111827", marginBottom: 2, fontFamily: "'Outfit',sans-serif" }}>
                                                        {step.label}
                                                    </div>
                                                    <AnimatePresence>
                                                        {isActive && (
                                                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                                                                style={{ fontSize: 12, color: "#6B7280", animation: "cc-pulse 1.4s ease infinite", fontFamily: "'Outfit',sans-serif" }}>
                                                                {step.sub}
                                                            </motion.div>
                                                        )}
                                                        {isDone && (
                                                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                                                                style={{ fontSize: 12, color: "#4F46E5", fontFamily: "'Outfit',sans-serif" }}>
                                                                Complete
                                                            </motion.div>
                                                        )}
                                                    </AnimatePresence>
                                                ...
                                                <AnimatePresence>
                                                    {isDone && (
                                                        <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 400 }}
                                                            style={{ width: 24, height: 24, borderRadius: "50%", background: "linear-gradient(135deg,#4F46E5,#7C3AED)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                                ...
                                <p style={{ fontSize: 12, color: "#C4CAD4", marginTop: 16, fontFamily: "'Outfit',sans-serif" }}>Usually takes 15–30 seconds</p>
                            </motion.div>
                        )}
                        ...
                        {phase === "done" && contestResult && (
                            <motion.div key="done"
                                ...
                                exit={{ opacity: 0 }} transition={{ duration: .5, ease: "backOut" }}
                                style={{ textAlign: "center" }}
                            >
                                <motion.div initial={{ scale: 0, rotate: -10 }} animate={{ scale: 1, rotate: 0 }}
                                    transition={{ type: "spring", stiffness: 300, damping: 18, delay: .1 }}
                                    style={{ display: "flex", justifyContent: "center", marginBottom: 24 }}>
                                    <div style={{ width: 80, height: 80, borderRadius: "50%", background: "linear-gradient(135deg,#EEF2FF,#F5F3FF)", border: "3px solid #4F46E5", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 34, boxShadow: "0 8px 32px rgba(79,70,229,.2)" }}>🎯</div>
                                </motion.div>

                                <motion.h2 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .2 }}
                                    style={{ fontSize: 28, fontWeight: 800, color: "#0F172A", margin: "0 0 8px", letterSpacing: "-0.6px", fontFamily: "'Outfit',sans-serif" }}>
                                    Contest is ready!
                                </motion.h2>
                                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: .3 }}
                                    style={{ color: "#6B7280", fontSize: 15, margin: "0 0 24px", lineHeight: 1.6, fontFamily: "'Outfit',sans-serif" }}>
                                    {contestResult.count} problems sealed.<br />
                                    {form.scheduledAt ? `Starts ${form.scheduledAt.fromNow()}.` : "Ready to start anytime."}
                                </motion.p>
                                ...
                                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .35 }}
                                    ...
                                    <span className="cc-stat-chip"><CodeOutlined style={{ fontSize: 11 }} />{contestResult.count} problems</span>
                                    <span className="cc-stat-chip"><ClockCircleOutlined style={{ fontSize: 11 }} />{form.duration / 60}h</span>
                                    <span className="cc-stat-chip"><ThunderboltOutlined style={{ fontSize: 11 }} />{form.minRating}–{form.maxRating}</span>
                                    {form.inviteEmails.length > 0 && (
                                        <span className="cc-stat-chip"><MailOutlined style={{ fontSize: 11 }} />{form.inviteEmails.length} invited</span>
                                    )}
                                    <span className="cc-stat-chip">
                                        {contestResult.visibility === "private"
                                            ? <><EyeInvisibleOutlined style={{ fontSize: 11 }} />Private</>
                                            : <><GlobalOutlined style={{ fontSize: 11 }} />Public</>}
                                    </span>
                                </motion.div>

                                <motion.div className="cc-card" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .4 }} style={{ padding: "24px 28px", textAlign: "left" }}>
                                    <div style={{ fontSize: 11, fontWeight: 700, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 12, fontFamily: "'Outfit',sans-serif" }}>Room Code</div>
                                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                        <div className="cc-room-code">{contestResult.contestId}</div>
                                        <Button
                                    ...
                                    <div className="cc-sealed">
                                        ...
                                        Problem names stay hidden from all participants until you hit Start in the host panel.
                                    </div>
                                    {form.inviteEmails.length > 0 && (
                                        <div style={{ marginTop: 12, padding: "10px 14px", borderRadius: 10, background: "#EEF2FF", border: "1px solid #C7D2FE", fontSize: 12, color: "#3730A3", display: "flex", gap: 8, alignItems: "flex-start", fontFamily: "'Outfit',sans-serif" }}>
                                            <MailOutlined style={{ flexShrink: 0, marginTop: 1 }} />
                                            Invitations sent to {form.inviteEmails.length} email{form.inviteEmails.length > 1 ? "s" : ""}.
                                        </div>
                                    )}
                                </motion.div>

                                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .5 }} style={{ display: "flex", gap: 10, marginTop: 14 }}>
                                    <Button className="cc-btn-out" onClick={resetForm}>Create another</Button>
                                    <Button className="cc-btn-pri" onClick={() => onContestCreated?.(contestResult.contestId)}>Open Contest Room →</Button>
                                </motion.div>
                            </motion.div>
                        )}
