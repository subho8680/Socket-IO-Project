...
export default function Navbar() {
    ...
    return (
        <header className="sticky top-0 z-50 flex items-center justify-between px-6 h-16 border-b border-bg-border"
            style={{ background: 'rgba(7,7,14,0.92)', backdropFilter: 'blur(12px)' }}>
            <Logo size="sm" />

            <div className="flex items-center gap-4">
                <div className="hidden sm:flex items-center gap-1 px-3 py-1 rounded-full border border-bg-border"
                ...
                <Badge count={3} size="small" color="#7c3aed">
                    <button className="w-9 h-9 rounded-lg flex items-center justify-center text-txt-secondary hover:text-txt-primary hover:bg-bg-hover transition-all"
                ...
                <Dropdown menu={{ items: menuItems }} trigger={['click']} placement="bottomRight">
                    <button className="flex items-center gap-2 px-2 py-1 rounded-xl hover:bg-bg-hover transition-all">
                        ...
                        <div className="hidden sm:block text-left">
                            <div className="text-xs font-semibold text-txt-primary leading-tight">{user?.name || 'User'}</div>
                            <div className="text-xs text-txt-secondary leading-tight">{user?.email || ''}</div>
                        </div>
                    </button>
                </Dropdown>
            </div>
